...
public class LineFragment extends Fragment {
    ...
    public void produceOne(LineChartView chart, Runnable action){
        ...
        LineSet dataset = new LineSet(mLabelsOne, mValuesOne[0]);       // 选择X轴标签，y轴
        dataset.setColor(Color.parseColor("#a34545"))                   // 线条颜色
                .setFill(Color.parseColor("#a34545"))                   // 背景颜色
                .setSmooth(true);                                       // 平滑曲线
        chart.addData(dataset);                                         // 线条添加到chartview中
        ...
        chart.setTopSpacing(Tools.fromDpToPx(15))           // chartview与父容器的padding值
                .setBorderSpacing(Tools.fromDpToPx(0))      // 左右边界padding
                .setAxisBorderValues(0, 10, 100)              // y轴坐标起始值，
                .setXLabels(AxisController.LabelPosition.INSIDE)// x轴标签可见
                .setYLabels(AxisController.LabelPosition.NONE)  // Y轴标签不可见
                .setLabelsColor(Color.parseColor("#e08b36"))    // x轴标签颜色
                .setXAxis(false)                            // x轴可见
                .setYAxis(false);                           // y轴不可见
        // 重绘前动画
    ...
    public void produceTwo(LineChartView chart, Runnable action){
        ...
        LineSet dataset = new LineSet(mLabelsTwo, mValuesTwo[0]);   // 折线1
        dataset.setColor(Color.WHITE)                       // 线条颜色
                .setThickness(Tools.fromDpToPx(3))          // 线条粗细
                .beginAt(4).endAt(55);                      // 显示的x范围，开始位置为4，结束位置为55
        chart.addData(dataset);                             // 添加到chartView中

        dataset = new LineSet(mLabelsTwo, mValuesTwo[1]);
        dataset.setColor(Color.parseColor("#97b867"))
                .setThickness(Tools.fromDpToPx(3))
                .setDashed(new float[]{10, 10});            // 设置成虚线样式，虚10实10
        chart.addData(dataset);

        Paint gridPaint = new Paint();                      // 网格画笔
        gridPaint.setColor(Color.parseColor("#7F97B867"));
        gridPaint.setStyle(Paint.Style.STROKE);
        gridPaint.setAntiAlias(true);
        gridPaint.setStrokeWidth(Tools.fromDpToPx(.75f));

        chart.setBorderSpacing(Tools.fromDpToPx(0))                 // 左右padding为0
                .setXLabels(AxisController.LabelPosition.OUTSIDE)   // x轴样式
                .setLabelsColor(Color.parseColor("#304a00"))        // x轴标签颜色
                .setYLabels(AxisController.LabelPosition.NONE)      // Y轴样式
                .setXAxis(false)
                .setYAxis(false)
                .setAxisBorderValues(0, 125, 25)                    // y轴的值为0~125，单位间距为25
                .setGrid(ChartView.GridType.HORIZONTAL, gridPaint); // 横轴网格
    ...
    public void produceThree(LineChartView chart, Runnable action){
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {   // 版本14
        ...
        chart.setBorderSpacing(1)
                ...
                .setYAxis(false)
//                .setStep(2)
                .setBorderSpacing(Tools.fromDpToPx(5))          // 垂直网格
