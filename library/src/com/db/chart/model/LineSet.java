...
public class LineSet extends ChartSet{
	...
	/** Defaults */
    private static final int DEFAULT_COLOR = -16777216;			// 默认颜色值
    private static final float LINE_THICKNESS = 4;				// 默认线条粗细值

	
	/** Line variables */
	private float mThickness;			// 线条粗细
	private int mColor;					// 线条颜色
	
	
	/** Line type 线条类型*/
	private boolean mIsDashed;			// 虚线
	private boolean mIsSmooth;			// 平滑


	/** Background fill variables */
	private boolean mHasFill;		// 是否填充背景
	private int mFillColor;			// 背景的颜色
	
	
	/** Gradient background fill variables */
	private boolean mHasGradientFill;		// 是否填充梯形背景颜色
	private int[] mGradientColors;			// 梯形颜色的值
	private float[] mGradientPositions;		// 梯形位置
	
	
	/** Index where set begins/ends */
	private int mBegin;			// 索引起始
	private int mEnd;			// 索引结束
	

    /** Intervals to apply in dashness 画虚线需要的参数*/
    private float[] mDashedIntervals;		// 虚线间隔
    /** Phase of the line (useful for animations) */
	private int mDashedPhase;				// 虚线间隔的偏移量
	...
	public void addPoint(Point point){
		this.addEntry(point);			// 图表集合添加point
	}
