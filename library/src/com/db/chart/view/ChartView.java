...
public abstract class ChartView extends RelativeLayout{
	...
	/** Horizontal and Vertical position controllers 水平、垂直的点的处理对象*/
	final XController horController;
	final YController verController;
	
	
	/** Chart data to be displayed 要显示的图表折线集合*/
	ArrayList<ChartSet> data;
	
	
	/** Style applied to chart 图表风格*/
	final Style style;
	
	
	
	/** Threshold limit line value 临界线值*/
	private float mThresholdValue;
	
	
	/** Chart data to be displayed 要显示的图表数据*/
	private ArrayList<ArrayList<Region>> mRegions;
	
	
	/** Index of last point clicked 最新的被点击处的索引*/
	private int mIndexClicked;			// 第几个点被点击
	private int mSetClicked;
	
	
	/** Listeners to for touch events 触摸事件的监听器*/
	private OnEntryClickListener mEntryListener;
	private OnClickListener mChartListener;

	
	/** Drawing flag 绘制flag，准备绘制*/
	private boolean mReadyToDraw;
	
	
	/** Drawing flag 绘制flag，正在绘制*/
	private boolean mIsDrawing;


	/** Chart animation 图表动画*/
	...
	private Tooltip mTooltip;	// 顶部控件，能够在点击折线时显示的对应数据
    ...
    final private OnPreDrawListener drawListener = new OnPreDrawListener(){
		...
		public boolean onPreDraw() {
			...
			verController.init();	// y轴初始化
			mThresholdValue = verController.parsePos(0, mThresholdValue);
			// Mandatory: X axis after Y axis!
			horController.init();   // x轴初始化
			
			// Process data to define screen positions 特殊加工过的数据，定义屏幕中的位置
			digestData();
			
			// Tells view to execute code before starting drawing 绘制前的操作
			onPreDrawChart(data);
			...
			if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.HONEYCOMB)
				ChartView.this.setLayerType(LAYER_TYPE_SOFTWARE, null);		// 动画性能优化
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if(mAnim == null || !mAnim.isPlaying())
			...
			if(event.getAction() == MotionEvent.ACTION_DOWN &&
				...
				//Check if ACTION_DOWN over any ScreenPoint region.	检查点击的位置是否落在任何一个折线点的范围内
	 ...
	 * {0, 2, 4, 6} as labels.
     * @param minValue   Y轴显示的最低值
	 * @param maxValue   The maximum value that Y axis will have as a label Y轴显示的最大值
